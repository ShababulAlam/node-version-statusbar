...
function activate(context) {
    ...
    // Register commands
    const refreshCommand = vscode.commands.registerCommand("nodeVersion.refresh", () => {
        nodeVersionProvider.refresh();
    });
    const copyVersionCommand = vscode.commands.registerCommand("nodeVersion.copyVersion", async () => {
        const version = await nodeVersionProvider.getNodeVersion();
        if (version) {
            await vscode.env.clipboard.writeText(version);
            vscode.window.showInformationMessage(`Node.js version ${version} copied to clipboard!`);
        }
    });
    context.subscriptions.push(refreshCommand, copyVersionCommand, nodeVersionProvider);
...
class NodeVersionProvider {
    ...
    updateStatusBar(version, errorMessage) {
        if (!version && !errorMessage) {
            ...
            const config = vscode.workspace.getConfiguration("nodeVersion");
            const template = config.get("statusBarText", "$(symbol-method) Node {version}");
